...
private:
    ...
    std::vector<Point> outerBorderPoints;
...
public:
    ...
    void buildClockFace() {
        ...
        // Draw outer golden/orange border (gradient effect with concentric circles)
        for (int r = (int)clockRadius; r >= (int)(clockRadius - 8); r--) {
            CircleDrawer::drawCircle((int)clockCenterX, (int)clockCenterY, r, outerBorderPoints);
        }

        // Draw main clock face circles
        CircleDrawer::drawCircle((int)clockCenterX, (int)clockCenterY,
                                 (int)(clockRadius - 10), clockFacePoints);
        CircleDrawer::drawCircle((int)clockCenterX, (int)clockCenterY,
                                 (int)(clockRadius - 15), clockFacePoints);

        // Draw inner circle
        CircleDrawer::drawCircle((int)clockCenterX, (int)clockCenterY,
                                 (int)(clockRadius * 0.58f), clockFacePoints);
        ...
        for (int i = 0; i < 12; i++) {
            ...
            // Outer mark position
            float outerX = clockCenterX + clockRadius * 0.82f * cos(angle);
            float outerY = clockCenterY + clockRadius * 0.82f * sin(angle);

            // Inner mark position
            float innerX = clockCenterX + clockRadius * 0.72f * cos(angle);
            float innerY = clockCenterY + clockRadius * 0.72f * sin(angle);

            // Thicker marks for 12, 3, 6, 9
            float thickness = (i % 3 == 0) ? 5.0f : 3.0f;
            LineDrawer::drawThickLine((int)outerX, (int)outerY,
                                     (int)innerX, (int)innerY, thickness, clockFacePoints);
        }
    }

    void renderHourNumbers() {
        // Render hour numbers 1-12 around the clock face
        for (int i = 1; i <= 12; i++) {
            float angle = ((i - 3) * 30.0f) * M_PI / 180.0f; // -3 to start from 12
            
            // Position numbers at 62% of clock radius
            float numberX = clockCenterX + clockRadius * 0.62f * cos(angle);
            float numberY = clockCenterY + clockRadius * 0.62f * sin(angle);

            glRasterPos2f(numberX - 5, numberY + 5);
            
            // Set color for numbers (white/light gray)
    ...
    void render() {
        ...
        glColor3f(1.0f, 0.6f, 0.2f); // Bright orange
        for (const auto& point : outerBorderPoints) {
            glVertex2f((float)point.x, (float)point.y);
        }

        // Draw clock face with blue/purple gradient effect
        glColor3f(0.3f, 0.2f, 0.6f); // Blue-purple
...
void display() {
    ...
    // Render hour numbers (outside of glBegin/glEnd)
    g_clockSystem->renderHourNumbers();
...
int main(int argc, char** argv) {
    ...
    std::cout << "Name: Nafis Fuad Barshan | Reg. Num: 21201166 | Section: B" << std::endl;
